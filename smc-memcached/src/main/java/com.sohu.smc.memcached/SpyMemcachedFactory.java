package com.sohu.smc.memcached;
...
public class SpyMemcachedFactory {
    ...
    public static synchronized void init() throws FileNotFoundException {
        final String env = ConfigurationManager.getDeploymentContext().getDeploymentEnvironment();
        URL url = SpyMemcachedFactory.class.getResource("/");
        File dir = new File(url.getPath());
        File[] memcachedXmls = dir.listFiles(new FilenameFilter() {
            @Override
            public boolean accept(File dir, String name) {
                String format;
                if (StringUtils.equals(env, "online")) {
                    format = "-memcache.xml";
                } else {
                    format = "-memcache-test.xml";
                }
                if (StringUtils.containsIgnoreCase(name, format)) {
                    return true;
                }
                return false;
            }
        });
        if (memcachedXmls == null || memcachedXmls.length == 0) {
            throw new FileNotFoundException("class path resource [*-memcache.xml] cannot be opened because it does not exist");
        }
        String[] xmlFiles = new String[memcachedXmls.length];
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < memcachedXmls.length; i++) {
            xmlFiles[i] = memcachedXmls[i].getName();
            sb.append(memcachedXmls[i].getName()).append(",");
        }
        SmcConfiguration.init();
        LOG.info("[smc-memcached]:Find redis configuration files:[{}]", sb.substring(0, sb.length() - 1));
        ctx = new ClassPathXmlApplicationContext(xmlFiles);
    }
...
//                clientSlaver = new MemcachedClient(new KryoTextConnectionFactory(), AddrUtil.getAddresses(bean.getSlaveAddress().replaceAll(",", " ")));
        ...
        if (bean.isFailover()) {
            try {
                CuratorFramework zkClient = CuratorFrameworkFactory.newClient(AppConfiguration.getString("smc.zookeeper.server", "10.10.76.79:2181").get(), new RetryOneTime(3));
                zkClient.start();
