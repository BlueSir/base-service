...
import com.nsq.oss.StringHelper;
...
public class ExampleMain {
    ...
    /*
       ...
       */
            //producer
            NSQProducer producer = new NSQProducer().addAddress("10.1.157.95", 4150, 1);
            producer.start();
            start = new Date();
...
//		My System does this in about 10 seconds, so 5k messages per second on a single connection
        ...
        lookup.addAddr("10.1.157.95", 4161);

        start = new Date();
        /**
         * Consumer.  consume 50k messages and immediately exit
         */
        NSQConsumer consumer = new NSQConsumer(lookup, "speedtest2", "dustin", new NSQMessageCallback() {
